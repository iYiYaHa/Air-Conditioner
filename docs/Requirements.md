...
| 功能名称 | 描述 |
|--|--|
| 风速调节 | 自动->低速->中速->高速 从控机接收用户设置，将请求发送给主控机，由主控机调节风速 |
| 温度调节 | 不同模式下（除送风外）温度调节范围相同，为 16-30 °C |
|送风模式 | 温度保持在 26 摄氏度 |
| 模式调节 | 模式分为制冷、制热、送风、干燥、辅热、定时、换气、睡眠、扫风，主控机接收 |

#### 身份验证、管理

| 功能名称 | 描述 |
|--|--|
| 管理员登录 | 管理员验证身份后，可查看空调使用详单，收益情况详单，控制主控机和从控机开关 |
|	主控机启动/关闭 | 管理员登录后可启动/关闭主控机 |
| 各房间从控机使用权限启动/关闭 | 管理员登录后，可根据客房入住情况，启动/关闭各房间从控机使用权限 |
...
|--|--|
| 风速调节 | 自动->低速->中速->高速 从控机接收用户设置，将请求发送给主控机，由主控机调节风速 |
| 温度调节 | 达到目标温度后从控机开始工作。制冷（热）模式下，当房间温度超过（低于）目标温度 1 °C时，重新启动。 |
| 面板温度调节 | 调节温度上升（下降）时长按按钮，每 0.5s 温度以 1°C 为单位增加或减少 |
| 模式调节 | 模式分为制冷 制热 送风 干燥 辅热 定时 换气 睡眠 扫风，使用者设置后从控机向主控机发送请求 |
| 打开和关闭空调 | 按动电源键，打开或者关闭空调，从控机接受用户设置，将请求发给主控机，由主控机控制打开和关闭空调 |
