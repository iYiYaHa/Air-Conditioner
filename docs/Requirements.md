...
> - v0.41 | 2017/4/05 | 董星彤 左旭彤 | 修改3. 产品应当遵循的标准或规范，增加0.4参考文档，修复5的小错误，增加能耗计算
## 0. 文档介绍 (10分)
### 0.1 文档目的 (2分)
...
| | 费用计算| 处理采集得到的数据，计算费用。 通过运行时间和风速计算出能耗和费用。低速功率为 2200W ，中速功率为 2400W ，高速功率为 2600w (每小时)。 |
| | 支付信息处理 | 向从控机发送详细的支付信息(支付包二维码或前台支付)，接收用户支付信息 |
| | 故障处理 | 宕机后保证数据不会丢失 |
| 风速调节 | 风速调节 | 自动->低速->中速->高速 从控机接收用户设置，将请求发送给主控机，由主控机调节风速 |
| 温度控制 | 各模式下（除送风外）温度调节范围 | 不同模式下（除送风外）温度调节范围相同，为 16-30 °C |
| |送风模式 | 温度保持在 26 摄氏度 |
...
| 风速调节 | 风速调节 | 自动->低速->中速->高速 从控机接收用户设置，将请求发送给主控机，由主控机调节风速 |
| 温度调节 | 温度与从控机工作关系规律 | 达到目标温度后从控机开始工作。制冷（热）模式下，当房间温度超过（低于）目标温度 1 °C时，重新启动。 |
| | 面板温度调节 | 调节温度上升（下降）时长按按钮，每 0.5s 温度以1°C为单位增加或减少|
